...
var ALLOWED_SHAPES = ["圆珠", "米珠", "方形珠", "菱形珠", "扁圆珠", "管珠"];
var DIRECTION_KEYS = ["airy", "flow", "focus"];
var DEFAULT_PALETTE = ["#F5F1E8", "#DDE8E4", "#B8CFCA", "#8FA9A1", "#607A72"];
var DARK_ATMOSPHERE_PATTERN = /暗黑|黑暗|幽暗|阴暗|昏暗|暗夜|幽深|阴森|暮色|无光/;
...
function normalizeDirections(value, title) {
  ...
  const defaults = {
    airy: { name: `${title}留白`, layout: "spaced", mainRatio: 0.8, accentRatio: 0.04, symmetry: "radial" },
    flow: { name: `${title}流光`, layout: "gradient", mainRatio: 0.64, accentRatio: 0.22, symmetry: "bilateral" },
    focus: { name: `${title}焦点`, layout: "cluster", mainRatio: 0.58, accentRatio: 0.25, symmetry: "bilateral" }
  };
  ...
  return DIRECTION_KEYS.map((key) => {
    ...
    const bands = {
      airy: { main: [0.76, 0.82], accent: [0.04, 0.08] },
      flow: { main: [0.6, 0.68], accent: [0.18, 0.24] },
      focus: { main: [0.55, 0.62], accent: [0.23, 0.26] }
    }[key];
    ...
    return { key, name: cleanText(found.name, fallback.name, 12), layout, symmetry, mainRatio, accentRatio };
  });
}
function normalizeTheme(data, originalTheme = "未命名主题") {
  ...
  const hasTrustedSemantic = Number.isFinite(semanticConfidence) && semanticConfidence >= 0.65;
  const semanticMatches = SEMANTIC_CONCEPTS.filter((concept) => concept.pattern.test(String(originalTheme)));
  const semanticAnchor = !hasTrustedSemantic && semanticMatches.length === 1 ? semanticMatches[0] : null;
  ...
  const requestedBrightness = semantic.brightness ?? data?.brightness;
  const trustedSemanticBrightness = hasTrustedSemantic ? requestedBrightness : null;
  const brightness = darkAtmosphere ? "low" : trustedSemanticBrightness ?? semanticAnchor?.brightness ?? requestedBrightness;
  const basePalette = normalizePalette(semanticAnchor?.palette ?? data?.palette ?? data?.colors);
  const palette = darkAtmosphere || brightness === "low" ? composeDarkPalette(basePalette) : basePalette;
  ...
  const saturation = semanticAnchor?.saturation ?? semantic.saturation ?? data?.saturation;
  const visualWeight = darkAtmosphere ? "heavy" : semanticAnchor?.visualWeight ?? semantic.visualWeight ?? data?.visualWeight;
  return {
    title,
    semantic: {
      subject: cleanText(semantic.subject, cleanText(originalTheme, title, 32), 32),
      atmosphere: cleanText(semantic.atmosphere, "自然", 24),
      brightness: ["low", "medium-low", "medium", "medium-high", "high"].includes(brightness) ? brightness : "medium-high",
      temperature: ["cool", "warm", "neutral"].includes(semantic.temperature) ? semantic.temperature : "neutral",
      confidence: Number.isFinite(Number(semantic.confidence)) ? Math.max(0, Math.min(1, Number(semantic.confidence))) : 0.5
    },
...
function toneForDarkAtmosphere(hex, targetLightness) {
  ...
  const tonedSaturation = saturation < 0.22
    ? Math.min(0.1, saturation * 0.6)
    : Math.max(0.16, Math.min(0.52, saturation * 0.92));
  return hslToHex(hue, tonedSaturation, targetLightness);
}
