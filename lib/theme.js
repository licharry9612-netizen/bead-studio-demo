...
function normalizeTheme(data, originalTheme = "未命名主题") {
  ...
  const palette = normalizePalette(semanticAnchor?.palette ?? data?.palette ?? data?.colors);
  ...
  const brightness = semanticAnchor?.brightness ?? data?.brightness;
  const saturation = semanticAnchor?.saturation ?? data?.saturation;
  const visualWeight = semanticAnchor?.visualWeight ?? data?.visualWeight;
  return {
    title,
...
function createLocalTheme(rawTheme) {
  ...
  const concepts = SEMANTIC_CONCEPTS.filter((concept) => concept.pattern.test(title));
  if (!concepts.length) {
  ...
  return normalizeTheme({
    ...
    palette: composePalette(concepts),
    forbiddenColors: primary.forbiddenColors,
    materials: merged("materials"),
    shapes: merged("shapes"),
    metal: concepts.at(-1)?.metal ?? primary.metal,
    brightness: hasNight ? "medium" : primary.brightness,
    saturation: primary.saturation,
    transparency: concepts.reduce((sum, concept) => sum + concept.transparency, 0) / concepts.length,
    visualWeight: hasNight && concepts.length > 1 ? "medium" : primary.visualWeight
  }, title);
}
