...
import { normalizeTheme } from "./theme.js?v=84";
var DESIGN_PROFILES = [
  { key: "flow", label: "小珠细节", structure: "small-bead", targetSizes: { main: 3, secondary: 3, accent: 3, focal: 4 } },
  { key: "focus", label: "异形结构", structure: "shaped-bead", targetSizes: { main: 4, secondary: 4, accent: 4, focal: 5 } },
  { key: "airy", label: "大珠质感", structure: "large-bead", targetSizes: { main: 6, secondary: 6, accent: 6, focal: 7 } }
];
var LAYOUT_VARIANT_COUNT = 4;
...
function matchCost(bead, role, targetColor, targetSize, theme) {
  ...
  const shapeCost = theme.shapes.includes(bead.shape) ? 0 : 10;
...
function chooseBead(library, theme, profile, role, targetColor, variation, excludedIds, seed) {
  ...
  const roleEligible = (bead) => {
    ...
    if (role === "spacer" || bead.shape === "隔珠") return false;
  ...
  const roleOffset = { main: 0, secondary: 1, accent: 2, focal: 3 }[role];
...
function assignRoles(library, theme, profile, variation, seed) {
  ...
  return { focal, main, secondary, accent };
}
function roleCounts(length, direction) {
  ...
  let secondary = usable - main - accent;
  if (secondary < 4) {
    main = Math.max(1, main - (4 - secondary));
    secondary = usable - main - accent;
  }
  if (secondary < 0) {
    accent = Math.max(4, accent + secondary);
    secondary = 0;
  }
  main = usable - accent - secondary;
  return { main, secondary, accent, focal: 1 };
}
...
function buildAiryPattern(roles, length, direction, layoutVariant = 0) {
  ...
  if (layoutVariant === 3) counts.secondary = Math.max(counts.secondary, Math.floor((length - 1) * 0.22));
  ...
  if (layoutVariant === 1) {
    ...
    fillFromOrder(pattern, roles.secondary, counts.secondary, dualArcOrder(length), { index: 0 });
  } else {
...
function buildFocusPattern(roles, length, direction, layoutVariant = 0) {
  ...
  const pattern = Array(length).fill(null);
  pattern[0] = roles.focal;
  const accentOffset = [0, .5, 1, 1.5][layoutVariant % 4] * (length - 1) / Math.max(1, counts.accent);
  fillEvenly(pattern, roles.accent, counts.accent, accentOffset);
  if (layoutVariant === 1) {
    fillFromOrder(pattern, roles.secondary, counts.secondary, focalOrder(length), { index: 0 });
  } else if (layoutVariant === 2) {
    fillFromOrder(pattern, roles.secondary, counts.secondary, opposingClusterOrder(length), { index: 0 });
...
function roleRatios(pattern) {
  ...
  return { main: ratio('main'), accent: ratio('accent') };
}
function scoreCandidate(theme, profile, pattern, direction = theme.directions.find(item => item.key === profile.key)) {
  ...
  const ratioError = Math.abs(ratios.main - direction.mainRatio) + Math.abs(ratios.accent - direction.accentRatio);
...
function hasStructuredColorPlacement(pattern, profile) {
  ...
  for (const [beadId, positions] of positionsById.entries()) {
    ...
    if (bead.material === '金属' && profile !== 'airy') continue;
...
function focalTransitionIsBalanced(pattern) {
  ...
  return rolesMatch && left.family === right.family && Math.abs(left.size - right.size) <= 1;
}
...
function visualCrossSection(bead) {
  const factor = ({ '米珠': 0.82, '扁圆珠': 0.84, '管珠': 0.56, '方形珠': 0.9, '菱形珠': 0.82 })[bead.shape] ?? 1;
...
function maximumCircularRun(pattern, predicate) {
  if (!pattern.length) return 0;
  let longest = 0;
  let current = 0;
  for (const bead of [...pattern, ...pattern]) {
    current = predicate(bead) ? current + 1 : 0;
    longest = Math.max(longest, current);
  }
  return Math.min(longest, pattern.length);
}
...
function candidatePassesHardRules(candidate) {
  ...
  if (pattern.some(bead => bead.shape === '隔珠' || bead.role === 'spacer')) return false;
  ...
  if (tubeCount / pattern.length > 0.18 || tubeCount === 1) return false;
  if (candidate.profile !== 'focus' && structuralRatio > 0.18) return false;
  if (candidate.profile === 'focus' && maximumCircularRun(pattern, bead => STRUCTURAL_SHAPES.has(bead.shape)) > 3) return false;
  ...
  pattern.forEach((bead, index) => {
    if (bead.role === 'focal' || bead.size === dominantSize) return;
  ...
  if (candidate.direction) {
    const ratios = roleRatios(pattern);
    if (candidate.profile === 'airy') {
      if (ratios.main < 0.7 || ratios.accent > 0.01) return false;
...
function compositionNote(profile, roles, layoutVariant) {
  if (profile.key === 'airy') {
    ...
    if (layoutVariant === 3) return `${roles.main.name}铺陈主体，${roles.secondary.name}形成双侧分区。`;
  ...
  if (profile.key === 'flow') {
    ...
    if (layoutVariant === 3) return `${roles.main.name}为底，${roles.secondary.name}双区展开，${roles.accent.name}规律间隔。`;
    return `${roles.main.name}为底，${roles.secondary.name}与${roles.accent.name}双向过渡。`;
  }
  if (layoutVariant === 1) return `${roles.accent.name}规律穿插，${roles.secondary.name}在焦点两侧收束。`;
  if (layoutVariant === 2) return `${roles.accent.name}间隔重复，${roles.secondary.name}形成双区呼应。`;
  if (layoutVariant === 3) return `${roles.accent.name}周向重复，${roles.secondary.name}形成双侧点缀。`;
  return `${roles.accent.name}与${roles.main.name}规律交替，${roles.focal.name}集中焦点。`;
}
...
function generateDesigns(rawTheme, library, options = {}) {
  ...
  const layoutVariant = Math.abs(Math.trunc(Number(options.generationVariant ?? seed))) % LAYOUT_VARIANT_COUNT;
  const selected = [];
  for (const [profileIndex, profile] of DESIGN_PROFILES.entries()) {
    const direction = theme.directions.find((item) => item.key === profile.key);
    const candidates = Array.from({ length: candidateCount }, (_, variation) => Array.from({ length: LAYOUT_VARIANT_COUNT }, (_, layoutOffset) => {
      const candidateLayout = (layoutVariant + layoutOffset) % LAYOUT_VARIANT_COUNT;
    ...
    const preferredPool = candidatePool.filter((candidate) => candidate.layoutVariant === layoutVariant);
    const rankingPool = preferredPool.length ? preferredPool : candidatePool;
    const ranked = rankingPool.map((candidate) => {
