...
import { normalizeTheme } from "./theme.js?v=82";
var DESIGN_PROFILES = [
...
function candidatePassesThemeRatios(candidate, theme) {
  ...
  const primaryFamilyRatio = mainBeads.filter((bead) => bead.family === primaryFamily).length / Math.max(1, mainBeads.length);
  const requiredPrimaryRatio = ["白/米", "灰/黑"].includes(primaryFamily) ? 0.62 : 0.75;
  if (!mainBeads.length || primaryFamilyRatio < requiredPrimaryRatio) return false;
  ...
  for (const [profileIndex, profile] of DESIGN_PROFILES.entries()) {
    ...
    const candidatePool = visiblyFresh.length
      ...
      : freshValid.length
        ...
        : hardValid.length
          ? hardValid
          : freshStructurallyValid.length
            ? freshStructurallyValid
            : structurallyValid;
    if (!candidatePool.length) throw new Error(`没有可用且结构协调的${profile.label}方案`);
...
function generateDesigns2(theme, library, options) {
  try {
    ...
    if (!String(error?.message ?? '').startsWith('没有可用于')) throw error;
    const availableLibrary = library.filter(bead => Number(bead.count ?? 1) > 0 && bead.suitableRoles?.length);
    return generateDesigns(theme, availableLibrary, options);
  }
}
